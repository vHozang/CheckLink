...
def _purge_cache():
    now = datetime.utcnow()
    for k in list(EXPORT_CACHE.keys()):
        if now - EXPORT_CACHE[k]["ts"] > EXPORT_TTL:
...
store1.com"></textarea>
        ...
        <div class="controls">
          ...
          <label class="control">Timeout (1–60s): <input type="number" step="1" name="timeout" value="{{ default_timeout }}"></label>
      ...
      <div class="footer">
        <div>Gợi ý: Trên Render không dùng proxy 127.0.0.1. Nếu cần, nhập SOCKS5 công khai/riêng. Tăng Timeout nếu gặp 429/403; yêu cầu lớn có thể cần chạy lại theo batch.</div>
      </div>
    </div>
  </div>
</body>
...
def index():
    default_use_proxy = os.environ.get(
        "USE_PROXY", "false").lower() in ("1", "true", "yes", "y")
    default_proxy = os.environ.get("PROXY_HOSTPORT", "127.0.0.1:60000")
    default_timeout = int(os.environ.get("TIMEOUT", "20"))
    default_timeout = max(1, min(default_timeout, 60))
...
def check():
    ...
    # SAFE timeout: default to env TIMEOUT when blank/invalid, and clamp 1..60s
    try:
        timeout_val = float(timeout) if timeout else float(
            os.environ.get("TIMEOUT", "20"))
    except Exception:
        timeout_val = float(os.environ.get("TIMEOUT", "20"))
    timeout_val = max(1.0, min(timeout_val, 60.0))

    results = check_links(collected, use_proxy=use_proxy,
                          proxy_hostport=proxy_hostport or None, timeout=timeout_val)
    ...
    EXPORT_CACHE[export_id] = {
        "ts": datetime.utcnow(),
        "results": results,
        "params": {"use_proxy": use_proxy, "proxy_hostport": proxy_hostport, "timeout": timeout_val},
    }

    default_use_proxy = use_proxy
    default_proxy = proxy_hostport or os.environ.get(
        "PROXY_HOSTPORT", "127.0.0.1:60000")
    default_timeout = int(timeout_val)
...
def export_results(export_id):
    ...
    if fmt == "json":
        ...
        import io as _io
        buf = _io.BytesIO()
        buf.write(_json.dumps({"exported_at": datetime.utcnow().isoformat(
        )+'Z', "count": len(results), "results": results}, ensure_ascii=False).encode("utf-8"))
        buf.seek(0)
    ...
    writer.writerow(["input_url", "normalized_url", "final_url",
                    "http_status", "classification", "error"])
    for r in results:
        writer.writerow([
            r.get("input_url", ""),
